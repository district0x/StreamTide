(ns streamtide.ui.components.infinite-scroll
  (:require
...
(defn infinite-scroll [props & [children]]
  "Infinite scroll based on react-infinite component.
   react-infinite, instead of keeping all elements in the DOM, it renders only DOM nodes that the user is able to
   see or might soon see. This is more efficient and it does not need to keep all items in the DOM
   However, it is important to properly specify the element-height as otherwise scrolling might look odd sometimes"
    ...
    (fn [props & [children]]
      (let [{:keys [:class
             ...
             :or {class "infinite-scroll"
                  ...
                  element-height 490
                  elements-in-row (if @(subscribe [::w-size-subs/mobile?]) 1 2)
        ...
        (into [react-infinite
               (r/merge-props
                 ...
                 {:class class
                  ...
                  :preload-additional-height (container-height-scale-factor 4)
                  :on-infinite-load (fn []
                                      (when (and has-more?
                                                 (not loading?))
