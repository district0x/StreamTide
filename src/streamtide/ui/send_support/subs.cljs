...
(re-frame/reg-sub-raw
  ...
  (fn [db [_ user-address coin donation-type amount-eth]]
    (let [contract-address (:coin/address coin)
      ...
      (case donation-type
        "toshi-mart" (dispatch-debounce {:delay 500 :key (dissoc data :amount-eth) :event [::events/compute-toshi-mart-price data]}))
      (reaction (get-in @db [:coin-conversion (keyword contract-address)])))))


(re-frame/reg-sub
  ::coin-conversion-in-progress?
  (fn [db [_ user-address contract-address]]
    (get-in db [:coin-conversion-in-progress (keyword user-address) (keyword contract-address)])))
